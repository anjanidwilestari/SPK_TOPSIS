          ...
          <table class="table table-striped table-bordered table-hover table-responsive">
            ...
            <tbody>
              ...
              foreach ($data as $nama => $krit) {
                ...
                foreach ($kriteria as $k) {
                  if ($status[$k] == 'Benefit') {
                    echo "<td align='center'>" . round(round(($krit[$k] / sqrt($nilai_kuadrat[$k])), 6),4) . "</td>";
                  } else {
                    echo "<td align='center'>" . round(round(1 - ($krit[$k] / sqrt($nilai_kuadrat[$k])), 6),4) . "</td>";
                  }
                }
                echo
                "</tr>\n";
            ...
            <tbody>
              ...
              foreach ($data as $nama => $krit) {
                ...
                foreach ($kriteria as $k) {
                  ...
                  if ($status[$k] == 'Benefit') {
                    $y[$k][$i - 1] = round(($krit[$k] / sqrt($nilai_kuadrat[$k])), 6) * $bobot[$k];
                    echo "<td align='center'>" . round($y[$k][$i - 1],4) . "</td>";
                  } else {
                    $y[$k][$i - 1] = round(1 - ($krit[$k] / sqrt($nilai_kuadrat[$k])), 6) * $bobot[$k];
                    echo "<td align='center'>" . round($y[$k][$i - 1],4) . "</td>";
                  }
                }
                echo
            ...
            <tbody>
              ...
              foreach ($data as $nama => $krit) {
                echo "<tr>
                            ...
                            <td>{$nama}</td>";
                foreach ($kriteria as $k) {
                  $V[$i - 1] = round(round(sqrt($dmin[$i - 1]), 6) / (round(sqrt($dmin[$i - 1]), 6) + round(sqrt($dplus[$i - 1]), 6)),4);
                }
                echo "<td>{$V[$i - 1]}</td></tr>\n";
                $sortir[$i - 1] = $V[$i - 1]; // Menyimpan nilai untuk pengurutan
              }
              arsort($sortir); // Mengurutkan array dalam urutan menurun (dari terbesar)
              ?>
            </tbody>
          </table>
        </div>
        ...
        <div class="panel-body">
          <table class="table table-striped table-bordered table-hover table-responsive">
            ...
            <tbody>
              <?php
              $i = 0;
              foreach ($sortir as $index => $value) {
                // Menampilkan data sesuai urutan setelah diurutkan
                $nama = key(array_slice($data, $index, 1));
                echo "<tr>
                              <td>" . (++$i) . "</td>
                              <th>A" . ($index + 1) . "</th>
                              <td>{$nama}</td>
